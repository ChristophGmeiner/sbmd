...
- store data from web APIs which is only live available in a structured way
...
The overall dag runs approx. 40 minutes. I inserted all the waiting tasks for stretching out the web requests. Basically all the gathering work is done with BashOperators calling the relevant Python scripts (see below).
...
#### sbahnmuc03.py
This script gathers data concerning trains. The requests are parallalized with the multiprocessing module.

#### sbahnmuc04_gmap.py
This script gathers data from the Google map API. Due to the fact this API gets very expensive the number of stations or start/destination combinations are reduced. The requests are parallalized with the multiprocessing module.
...
After that - using the first three scripts below - the data is transformed to a Pandas dataframe, bulk copied to a Redshift staging table, inserted into the respective live tables and finally all transferred json files are archived into a newly created and time-stamped folder in the respective S3 bucket.
...
- Using the first three (03b, 04b, 05b) scripts below - the data is transformed to a csv file and succesfully transferred json files are archived into a newly created and time-stamped folder in the respective S3 bucket (the folder name indicates the date of the archving process).
...
##### Station names
This is done in the mentioned below 02 script. Since the API only operates on station names stored in strings, I have to gather the exact naming for the desired stations. Therefore a list with city and or street names is provided for this API to gather the DB API names of the relevant stations.
...
##### Connection details
This is done in the mentioned below 03 script and is gathering now all avaiable connection data for the station pairs created in the previous step.
...
- departure: Departure of connection
- arrival: Arrival of connection
- transfers: Number of different transfers for the specific connection
- time: Taken time for connection
- products: what kind of transportation methods are used for the connection (e.g. suburban train, subway, bus...)
...
One little difference: Due to the pricing policy of the google maps api, the amount of start and end locations has been reduced to only city names (no specific streets or similar).
...
#### Weather data
The weather data is gathered twice per hour for each locattion, which is relevant for the other two data gathering processes above. It is carried out with the pyowm module and the 05 script below. Please see the link above for more details on that. See an example json below:
...
Python script for creating live and staging tables during initial database creation.

#### zzCreateRedC.py
Creates a Amazon Redshift cluster from a snapshot. During the script the latest snapshot gets defined.

#### zzDelRedC.py
Deletes the cluster and creates a final snapshot with a specified name, so that during creation process the latest snapshot can be found and re-created.

#### airflow helpers

##### InsertTables.py
This script contains the necessary SQL scripts for transferring data between the Amazon Redshift staging and live tables.

#### CreateModelTables.py
This script contains the necessary SQL scripts for creating the initial tables for the data model (see details on the data model objects below).

#### DataModel.py
This script contains SQL scripts, for creating the fact and dimension tables for the anaöytical final data model (see below for information on that).
...
#### modifyrds.py
Similar to the previous one , but to be used with RDS services instead of Redshift.
...
#### csvs3postgres.py
Similar to the previous one, but only suitbale for postgres databases.

## DWH creation step 2 and data modelling 

### Tables in Amazon Redshift
Basically for all three data sources there is always one staging and one live table. The table names indicate what data source is covered by it. The staging tables contain the pure json/csv data without any formatting indexing etc of one loading process. 
The live tables contain all the staged data cumulated for all loading processes, but these already have proper data types and sorting and distribution keys or styles. These tables are the basis for the modelling process and contain the same fields and information as the json files.
Please see the airflow helpers scripts for details on the tables.

The main DWH model aims at answering all sort of questions on the traffic all around the Greater Munich area, e.g. when are you faster by train? What weather conditions benefits the car? etc.
...
### Other redshift objects

#### zz_tables
These zz_city tables contain standardized city names and links to start or destination location for all three data sources. These data gets incorprated into the dimnesion and fact tables.

#### t_w02_weather_codes
Matching of integer based weather codes to weather descriptions

#### Views and stored procedures
These objects are basically for data transformation puposes between the loading and modelling part.
    
### Files and scripts

#### CreateProcs.py
SQL query for initially creating a stored procedure for data loading.

#### CreateViews.py
SQL query for initially creating some views for data loading.

#### CreatezzTables.py
SQL query for initially creating some helper tables for data loading.
